<script>
  import FacebookSignInButton from "./social/FacebookSignInButton.svelte";
...
>
  <div class="max-w-md w-full space-y-8 p-6 rounded-xl shadow-lg bg-white">
    ...
    <form class="mt-8 space-y-6" on:submit|preventDefault={createAccount}>
      ...
      {/if}
      <div>
        <Button class="w-full" type="submit">Zarejestruj</Button>
      </div>
    </form>
    <div class="relative">
      ...
      <div class="relative flex justify-center text-xs uppercase">
        <span class="bg-white px-2 text-[#71717a]">Lub kontynuuj z</span>
      </div>
    </div>
    <div>
